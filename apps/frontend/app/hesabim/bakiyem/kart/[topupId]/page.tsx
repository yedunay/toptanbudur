...
type LoadState =
  | { kind: "loading" }
  | { kind: "ok"; data: PaytrTopupTokenData }
  | { kind: "error"; message: string };

export default function BakiyeKartOdemePage() {
  return (
    <Suspense fallback={<main className="mx-auto max-w-3xl px-6 py-10" />}>
      <BakiyeKartOdemePageInner />
    </Suspense>
  );
}

function BakiyeKartOdemePageInner() {
  ...
  useEffect(() => {
    ...
    apiCustomer<{ success?: boolean; data?: PaytrTopupTokenData }>(
      "/payments/card/topup-token",
      {
        method: "POST",
        general: true,
        ...
        <>
          <p className="mt-1 text-sm text-[var(--text-muted)]">
            ...
            <span className="font-mono font-black text-[var(--text)]">
              {state.data.humanTopupNo ?? state.data.topupId.slice(0, 8).toUpperCase()}
            </span>
          </p>

          <div className="mt-4 space-y-1.5 rounded-2xl border border-[var(--border)] bg-white p-4 text-sm shadow-sm">
            <div className="flex items-center justify-between font-semibold text-[var(--text)]">
              <span>Cari’ye eklenecek</span>
              <span>{formatPrice(state.data.amount, "TRY")}</span>
            </div>
            {state.data.commissionAmount > 0 ? (
              <div className="flex items-center justify-between text-xs font-medium text-[var(--text-muted)]">
                <span>Kart komisyonu</span>
                <span>+{formatPrice(state.data.commissionAmount, "TRY")}</span>
              </div>
            ) : null}
            <div className="flex items-center justify-between border-t border-[var(--border)] pt-1.5 font-black text-[var(--brand-navy)]">
              <span>Karttan çekilecek</span>
              <span>{formatPrice(state.data.chargedAmount, "TRY")}</span>
            </div>
          </div>
